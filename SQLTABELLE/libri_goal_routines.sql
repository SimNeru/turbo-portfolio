...
/*!50001 VIEW `editori_quanti_libri` AS select `e`.`nome` AS `Editore`,count(`e`.`nome`) AS `Quantità` from (`libri` `l` join `editori` `e` on((`l`.`editore_id` = `e`.`editore_id`))) group by `e`.`nome` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`app_goal`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `editori_nolibri` AS select `e`.`nome` AS `Editori` from (`editori` `e` left join `libri` `l` on((`e`.`editore_id` = `l`.`editore_id`))) where (`l`.`editore_id` is null) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
